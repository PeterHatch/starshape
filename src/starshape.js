...
function showControls(...visibleControls) {
  for (let [controlName, control] of controls) {
...
function calculateRadius(a, b, c) {
  const angle_cab = calculateAngle(a, b);
  const angle_cax = angle_cab + (Math.PI / 2);
  const length_am = calculateDistance(a, c) / 2;
...
class CubicStar extends InnerAndIntermediateStar {
  ...
  points(innerRadius, controlPercentage) {
    const [inner, control1, control2, outer] = super.points(innerRadius, controlPercentage);
